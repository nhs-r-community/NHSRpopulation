...
imd_notts_jitter <- imd %>%   
  ...
  ggtitle("Raw IMD scores for LSOAs in Notts")
...
The figure above illustrates that the the distribution of IMD deciles calculated based on the whole country (see Panel A) don't reflect the variation of the raw IMD scores within the Nottingham (see Panel B).
As mentioned above, comparing these side by side is a bit tricky because the two metrics (raw scores vs decile ranks) are scored in opposite directions.
