...
function.
Without specifying the different named column the following messages will show:

```{r, eval=FALSE}
NHSRpopulation::get_data(different_names, url_type = "postcode")
NHSRpopulation::get_data(different_names, url_type = "imd")
```

And the following parameter can be used to set the column name:

```{r, eval=FALSE}
# [TODO] returns errors but tests don't - needs debugging
